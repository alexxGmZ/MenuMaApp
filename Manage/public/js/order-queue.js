	...
	fetch('http://localhost:8080/orders')
		...
		.then(data => {
			...
			data.forEach(orders => {
				...
				const markup = `
					<td>
						<table class="border-separate border-spacing-2 border border-slate-500 rounded-lg bg-amber-200">
						...
						<table>
							<ul>
								...
								<li>
									<center>
										<button onclick="location.reload()" class="font-bold rounded-full bg-red-500 mt-2 py-2 px-2 hover:text-zinc-50 hover:drop-shadow-lg w-3/4 flex items-center justify-center">
